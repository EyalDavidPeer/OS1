...
Command *SmallShell::CreateCommand(const char *cmd_line) {
    ...
    if (SmallShell::getInstance().isAlias(firstWord)) {
        ...
        string real_name = SmallShell::getInstance().getRealNamefromAlias(
                firstWord);

        //replace alias with real name
        string line_without_first_word = cmd_s.substr(firstWord.length(),
                                                      cmd_s.length());
        char *real_line = new char(
                cmd_s.length() + (real_name.length() - firstWord.length()));
...
//    std::string cmd_line = _trim(cmd_line);
//
//    //TODO: replace the _parse method to _parseCommandLine method provided
//    std::vector<std::string> args = _parse(cmd_line);
//
...
//    int job_id = -1;
//    string cmd_line = _trim(cmd_line);
//
//    //TODO: replace the _parse method to _parseCommandLine method provided
//    std::vector<std::string> args = _parse(cmd_line);
//
...
void AliasCommand::execute() {
    ...
    cmd_s = _trim(tmp);
    string secondWord = cmd_s.substr(0, cmd_s.find_first_of("='"));
    if (SmallShell::getInstance().isSaved(secondWord) ||
        SmallShell::getInstance().isAlias(secondWord)) {
        cerr << "smash error: alias: " << secondWord
             << " already exists or is a reserved command" << endl;
        return;
    }

    //add alias to aliases map
    string real_name = cmd_s.substr(cmd_s.find_first_of('\'') + 1,
                                    cmd_s.length());
...
void UnSetEnvCommand::execute() {
    ...
    while (true) {
        ...
        if (vars.find(unwanted_var) == vars.end()) {
            ...
            cerr << "smash error: unsetenv: " << unwanted_var
                 << " does not exist" << endl;
            return;
        }
...
void ExternalCommand::execute() {
    ...
    if (cmd_s.find_first_of('*') != string::npos ||
        cmd_s.find_first_of('?') != string::npos) {
        executeComplex();
        return;
    }
